...
def get_text_messages(message):
    ...
    на вход функция принимает сообщение от пользователя(переменную message типа str)"""
...
def wep_id(message):
    ...
    """функция получает сообщение от пользователя содержащее наимневание искомого оружия и значение присваивается глобальной переменной weapon типа str
    (на вход принимается message типа str)"""
...
def wep_sk(message):
    ...
    guns_vs5) и их актуальные цены(guns_vs2,guns_vs4,guns_vs6),а так жекартинку самого предмета (kartinka)(на вход принимается message типа str)"""
...
def links():
    ...
    названия и раскраски оружия в строке, обозначающей ссылку. Результат достигается с помощью метода replace(). Итогом становится
...
def get_html(url, params=None):
        ...
        """функция на вход получает url страницы с которой необходимо получить контент. далее выполняется get запрос, который записывается в 
        в переменную r и в конце своей работы, функция возвращает эту переменную"""
...
def parse():
    ...
    или сообщает об ошибке подлючения"""
