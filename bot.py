...
def links():
    ...
    """"Методом replace изменяется исходное значение в переменной URL. Функция возвращает извлечённые данные. 
    Данные представены в виде кортежа, в котором находятся 7 элементов, где
    guns1[0] - первый элемент списка (наименование первого найденного пердмета)
    guns2[0] - первый элемент списка (стоимость первого найденного предмета)
    guns3 - строка (наименование прдмета отличающееся от наименования первого предмета. т.е предмет другого качества)
    guns4 - строка (стоимость предмета, наимнование которого указано в guns3)
    guns5 - строка (наименование прдмета отличающееся от наименования пердмета, указзаного в guns3)
    guns6 - строка (стоимость предмета, указанного в guns5)
    kartinka - строка (ссылка на картинку искомого предмета)
    """
